...
void Shader::SetVec3(const std::string& name, const std::vector<glm::vec3>& value)
{
	glUniform3fv(glGetUniformLocation(_programID, name.c_str()), value.size(), &value[0][0]);
}

void Shader::SetVec4(const std::string& name, const std::vector<glm::vec4>& value)
{
	glUniform4fv(glGetUniformLocation(_programID, name.c_str()), value.size(), &value[0][0]);
}
